...
supported_cuda_versions = {
	{ name = "13.0", envvar = "CUDA_PATH_V13_0", path_linux = "/usr/local/cuda-13.0", path_windows = "C:\\Program Files\\NVIDIA GPU Computing Toolkit\\CUDA\\v13.0" },
	{ name = "12.2", envvar = "CUDA_PATH_V12_2", path_linux = "/usr/local/cuda-12.2", path_windows = "C:\\Program Files\\NVIDIA GPU Computing Toolkit\\CUDA\\v12.2" },
}
...
for _, cuda_version in ipairs(supported_cuda_versions) do
	table.insert(supported_cuda_names, cuda_version.name)

	-- try the envvar
	if (not oropremake_PathOK(cuda_path)) then
		cuda_path = os.getenv(cuda_version.envvar)
	end

	-- if not found in envvar, try if path exist ( linux )
	if (not oropremake_PathOK(cuda_path)) then
		if ( os.isdir(cuda_version.path_linux) ) then
			cuda_path = cuda_version.path_linux
		end
	end

	-- try the windows path
	if (not oropremake_PathOK(cuda_path)) then
		if ( os.isdir(cuda_version.path_windows) ) then
			cuda_path = cuda_version.path_windows
		end
	end
end

if (not oropremake_PathOK(cuda_path)) then
	print("No supported version of CUDA for this Orochi is found: " .. table.concat(supported_cuda_names, " or ") .. ". It's advised that you install one of these versions.")
end
