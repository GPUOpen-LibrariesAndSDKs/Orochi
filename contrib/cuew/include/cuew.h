...
typedef cudaError_t CUDAAPI tcudaGLGetDevices(unsigned int * pCudaDeviceCount, int * pCudaDevices, unsigned int cudaDeviceCount, unsigned int deviceList);
typedef cudaError_t CUDAAPI tcudaGraphicsGLRegisterBuffer(cudaGraphicsResource_t * resource, unsigned int buffer, unsigned int flags);
...
extern tcudaGLGetDevices *cudaGLGetDevices_oro;
...
// CUDA 13 replaced cudaStreamGetCaptureInfo_v2 with an unsuffixed entry point that takes an extra
// edgeData_out argument, so it cannot reuse the generated _v2 typedef above.
